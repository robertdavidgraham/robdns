 ...
 * function prototypes
 */
typedef struct pfring*(*PFRING_OPEN)(
                    const char *device_name,
                    unsigned caplen,
                    unsigned flags);
typedef void (*PFRING_CLOSE)(struct pfring *ring);
typedef int (*PFRING_ENABLE_RING)(struct pfring *ring);
typedef int (*PFRING_SEND)(struct pfring *ring,
                    const unsigned char *buffer,
                    unsigned buffer_length,
                    unsigned char flush_packet);
typedef int (*PFRING_RECV)(struct pfring *ring,
                    unsigned char** buffer,
                    unsigned buffer_length,
                    struct pfring_pkthdr *hdr,
                    unsigned char wait_for_incoming_packet);
typedef int (*PFRING_POLL)(struct pfring *ring, unsigned wait_duration);
typedef int (*PFRING_VERSION)(struct pfring *ring, unsigned *version);
typedef int (*PFRING_SET_DIRECTION)(struct pfring *ring, int direction);
typedef int (*PFRING_SET_APPLICATION_NAME)(struct pfring *ring, char *name);
typedef int (*PFRING_GET_BOUND_DEVICE)(struct pfring *ring, unsigned char mac_address[6]);
